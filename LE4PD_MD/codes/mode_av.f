...
!	calculating instantaneous mode vector
	do a=4,20 !only looking at this mode for now !mode loop
...
!	calculate theta, phi
	do a=4,20 !only looking at this mode for now
...
c	if(a.eq.4)write(*,*)itheta,iphi,hisang(a,itheta,iphi)
	...
	do j=1,n !residue loop for av structure
...
c	write(jj,*)j
	jj=adjustl(jj)	
c	write(*,*)"phi:",trim(jj)
	open(unit=100,file='av_m'//trim(aa)//'_theta'//
