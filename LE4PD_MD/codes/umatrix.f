...
c	write(*,*)lavm(i)
	...
	subroutine nlist(ntot)
	integer i,j,k,nn
	integer,dimension(ntot) :: nloc
	integer,dimension(ntot,ntot) :: neighlist
	real,dimension(ntot) :: rij
	nn=4
	...
	do j=1,ntot
	read(15,*)rij(j)
	if(rij(j).eq.0.0)rij(j)=.001
	rij(j)=1.0/rij(j)
c	if(i.eq.2)write(*,*)rij(j)
	end do	
	do k=1,nn
	nloc=minloc(rij)
	neighlist(i,k)=nloc(1)
	rij(nloc(1))=500.0
	end do
	nloc=0
	rij=0.0
	end do
	open(unit=16,file='neighlist.dat')
	do i=1,ntot
	write(16,*)neighlist(i,1),neighlist(i,2),
     &neighlist(i,3),neighlist(i,4)
	end do
	close(16)
	end subroutine
