	...
	integer i,j,k,ntot,nc,nmol
	real avbl,bl
	character(16)crap,protname,cnc
	character(128)line
	real, dimension(:), allocatable :: resrad,avresrad,length,avlength
	...
	read(2,*)nmol
	close(2)
	ALLOCATE ( resrad(ntot),avresrad(ntot),avlength(ntot),length(ntot) )
	...
	open(unit=15,file='length'//trim(cnc),status='old')
	...
	do j=1,ntot-nmol
	read(15,*)length(j)
	avlength(j)=avlength(j)+length(j)
	end do
	...
	avbl=avbl/real(nc)
	do j=1,ntot-nmol
	avlength(j)=avlength(j)/real(nc)
	end do

	do j=1,ntot-nmol
	do k=1,ntot-nmol
	avum(j,k)=avum(j,k)/real(nc)
	avum(j,k)=avum(j,k)/(avlength(j)*avlength(k)*100) !100 because um units were from pdb in ang, and bond lengths in nm
