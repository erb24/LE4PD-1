...
C     CALCULATION OF FILE P2(T),TAU        
	...
	dimension qinvm(n-1,n-1),qm(n-1,n-1),eigmu(n-1),
     & eiglam(n-1),dot(n,n)
...
!	close(66)
   ...
   55 continue
c
	open(unit=61,file='avm1CArot',status='unknown')
	do l=1,index
	tt(l)=tc(l)/sg
