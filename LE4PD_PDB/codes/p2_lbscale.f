...
!	write(*,*)dot(i,j)
	...
	barrier=0.0
	do i=4,nm1
!	barrier(i)=Rb*Tg*exp(2/(eigmu(i)**.5))
!	barrier(i)=Rb*Tg*(1+(3.5/(eigmu(i)**.5)))
c	if(barrier(i).ge.10.0)barrier(i)=10.0
!	do j=1,nm1
!	barrier(i)=barrier(i)+((qm(j,i)**2)/eigmu(i))**.5
!	end do
!	barrier(i)=Rb*Tg*barrier(i)
	ip=i-3
	eigmuscale=(eigmu(4)*real(ip))
	barrier(i)=((blsq/(eigmuscale))**.5)*eps
!	barrier(i)=(1.6*(eigmu(i)**(-.5)))
