	...
	call pdbmaker(start,ntot,nr)
	end program

	subroutine pdbmaker(start,ntot,nr)
	...
	real c4x(ntot),c4y(ntot),c4z(ntot),c2x(ntot),c2y(ntot),c2z(ntot)
...
c	end do
	...
	write(cmode,*)mode
	cmode=adjustl(cmode)
	open(unit=14,file="m"//trim(cmode)//"_mincmp_."
     &//trim(cframe)//".pdb")
	nres=0
	do i=1,start
	Read(13,*)
...
c	write(*,*)anum
	...
	write(posx,'(F6.3)')c4x(nres)
	write(posy,'(F6.3)')c4y(nres)
	write(posz,'(F6.3)')c4z(nres)
	posx=adjustl(posx)
	posy=adjustl(posy)
	posz=adjustl(posz)
	line(33:38)=posx(1:6)
