	...
	double precision work((ntot-1)*(ntot-1)),eiglam(ntot-1),eigmu(ntot-1),eignu(ntot-1),Atest(ntot-1,ntot-1),UILI(ntot-1,ntot-1)
	...
	double precision eiglamO(ntot-1),eigmuO(ntot-1),eignuO(ntot-1)
	character(32)protname
	...
	nm1=n-1
	blsq=0.0
	open(unit=19,file='avblsq',status='old')
	read(19,*)blsq
	close(19)
...
!	end do
	...
	open(unit=10,file="UILImatrix",status='unknown')
	do i=1,nm1
        do j=1,nm1
	write(10,*)UILI(i,j)
	end do
	...
	eiglam=0.0
!	call DSYEV("V","U",nm1,UILI,nm1,eiglam,work,nm1*nm1,io)
!	write(*,*)"ssyev: (0 is success)",io,work(1)
	call DGEEV("N","V",nm1,UILI,nm1,eiglam,eiglamI,Atest,nm1,QM,nm1,work,nm1*nm1,io)
