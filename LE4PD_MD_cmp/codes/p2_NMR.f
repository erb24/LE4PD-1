...
C     CALCULATION OF FILE P2(T),TAU        
	...
	integer ai1,ai2,ai3,ai4,fintime,k,l,i,j,ip,nmol,nres(nmol)
	...
	character(32)protname,ii,cnmol
...
!	open(unit=789,file='avlength',status='old')
	open(unit=114,file='protname.txt',status='old')
	read(114,*)protname
	close(114)
...
!	write(*,*)dot(i,j)
	...
	write(ii,*)i
	ii=adjustl(ii)
	open(unit=110,file='mlength_'//trim(ii))
	do j=1,nm1
	if(j.eq.nt)then
	nt=nt+nres(i)
	im=im+1
	end if
	write(110,*)j+im,(((blsq*qm(j,i)**2)/eigmu(i))**.5)*10 !mode fluclength in ang
	end do
	close(110)
	end do
